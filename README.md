...
Lontar is a Rust-native library for generating documents across multiple output formats from a single, unified document model. Write your document once — in any script, any language — and render it to DOCX, PPTX, PDF, HTML, Markdown, or plain text.
...
let doc = Document::new("Quarterly Report")
    ...
    .paragraph("Laporan ini juga tersedia dalam bahasa Indonesia.")
    .paragraph("ᬮᭀᬦ᭄ᬢᬭ᭄ — ancient manuscripts, modern documents.")
    .table(
        &["Metric", "Q1", "Q2", "Q3"],
        &[
...
3. **Progressive fidelity.** Common operations (headings, paragraphs, tables, images, charts) work across all formats. Format-specific features (e.g., slide transitions in PPTX, tracked changes in DOCX) are available through format-specific extensions.

4. **Type-safe but ergonomic.** Rust's type system should prevent invalid documents at compile time, but the builder API should feel natural — not like wrestling with generics.

5. **Spec-compliant, not spec-complete.** We target correctness for the features we implement, following ECMA-376 (OOXML) and ISO 32000 (PDF). We don't aim to cover every obscure feature on day one.

6. **Zero unsafe, minimal dependencies.** Core document model uses no unsafe code. Format backends depend on `zip`, `quick-xml`, and format-specific crates only as needed.
