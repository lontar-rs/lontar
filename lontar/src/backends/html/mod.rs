...
impl<W: Write> HtmlWriter<W> {
    ...
    pub fn new(output: W) -> Self {
        Self { output, in_paragraph: false }
    }
        ...
        match block {
            ...
            Block::CodeBlock { code, language, .. } => {
                if let Some(lang) = language {
                    ...
                    writeln!(self.output, "<pre><code>")
                        .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                }
                write!(self.output, "{}", html_escape(code))
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                writeln!(self.output, "</code></pre>")
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
            }
            Block::BlockQuote { content, .. } => {
                writeln!(self.output, "<blockquote>")
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
            ...
            Block::HorizontalRule { .. } => {
                writeln!(self.output, "<hr>")
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
            }
            Block::Chart { .. } => {
                // Charts not rendered in HTML backend yet.
                writeln!(self.output, "<div class=\"chart-placeholder\">[Chart omitted]</div>")
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
            }
            Block::Image { resource_id, alt_text, width, height, .. } => {
                ...
                img_tag.push('>');
                writeln!(self.output, "{img_tag}")
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
            }
            Block::Equation { content, display, label, .. } => {
                if *display {
                    writeln!(self.output, "<div class=\"equation\">")
                        .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                    writeln!(self.output, "$${content}$$")
                        .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                    if let Some(lbl) = label {
                        writeln!(self.output, "<span class=\"label\">({lbl})</span>",)
                            .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                    }
                    writeln!(self.output, "</div>")
                        .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                } else {
                    write!(self.output, "<span class=\"equation\">${content}$</span>")
                        .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                }
            }
            Block::Section { title, content, .. } => {
                writeln!(self.output, "<section>")
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                if let Some(t) = title {
                    writeln!(self.output, "<h2>{}</h2>", html_escape(t))
                        .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                }
                for block in content {
                    self.write_block(block)?;
            ...
            Block::Bibliography { title, .. } => {
                writeln!(self.output, "<section class=\"bibliography\">")
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                if let Some(t) = title {
                    writeln!(self.output, "<h2>{}</h2>", html_escape(t))
                        .map_err(|e| WriteError::IoError { message: e.to_string() })?;
    ...
    fn write_inline(&mut self, inline: &Inline) -> WriteResult<()> {
        match inline {
            Inline::Text(text) => {
                write!(self.output, "{}", html_escape(text))
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
            }
            Inline::SoftBreak => {
                write!(self.output, " ")
            ...
            Inline::LineBreak => {
                writeln!(self.output, "<br>")
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
            }
            Inline::Bold(content) => {
                write!(self.output, "<strong>")
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                self.write_inline_sequence(content)?;
                write!(self.output, "</strong>")
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
            ...
            Inline::Code(code) => {
                write!(self.output, "<code>{}</code>", html_escape(code))
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
            }
            Inline::Strikethrough(content) => {
                write!(self.output, "<del>")
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                self.write_inline_sequence(content)?;
                write!(self.output, "</del>")
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
            ...
            Inline::Image { resource_id, alt_text, title } => {
                let alt = alt_text.as_deref().unwrap_or("");
                let title_attr = title
                    .as_ref()
                    .map(|t| format!(" title=\"{}\"", html_escape(t)))
            ...
            Inline::Styled { content, style } => {
                write!(self.output, "<span class=\"{}\">", html_escape(style))
                    .map_err(|e| WriteError::IoError { message: e.to_string() })?;
                self.write_inline_sequence(content)?;
...
impl<W: Write> DocumentWriter for HtmlWriter<W> {
    fn write(&mut self, document: &Document) -> WriteResult<WriteReport> {
        ...
        if let Some(title) = &document.metadata.title {
            writeln!(self.output, "  <title>{}</title>", html_escape(title))
                .map_err(|e| WriteError::IoError { message: e.to_string() })?;
        }
        ...
        if let Some(title) = &document.metadata.title {
            writeln!(self.output, "<h1>{}</h1>", html_escape(title))
                .map_err(|e| WriteError::IoError { message: e.to_string() })?;
        }
        ...
        if let Some(bib) = &document.bibliography {
            writeln!(self.output, "<section class=\"bibliography\">")
                .map_err(|e| WriteError::IoError { message: e.to_string() })?;
            writeln!(self.output, "<h2>References</h2>")
                .map_err(|e| WriteError::IoError { message: e.to_string() })?;
            writeln!(self.output, "<ol>")
                .map_err(|e| WriteError::IoError { message: e.to_string() })?;
